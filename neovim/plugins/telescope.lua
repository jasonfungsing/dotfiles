...
-- Modern fuzzy finder with enhanced features
...
telescope.setup({
  defaults = {
    prompt_prefix = " ",
    -- Single-cell caret + matching 2-cell prefix: a double-width icon here
    -- makes the selected line appear to "indent" as the selection moves
    selection_caret = "▌ ",
    entry_prefix = "  ",
    path_display = { "truncate" },
    file_ignore_patterns = { "%.git/", "node_modules/", "%.cache/" },

    -- Search prompt at the top, results below it top-to-bottom —
    -- matching the shell shortcut sheet (fzf --reverse)
    ...
    mappings = {
      i = {
        ...
        -- Tab / Shift+Tab navigate, matching the shell shortcut sheet
...
-- Searchable shortcut sheet (Ctrl+/): normal-mode keymaps only (the ones
-- executable from here), <Plug> noise hidden, grouped into sections. Three
-- columns — section tag, key, description (or the mapped command). The tag
-- is part of each row so it survives filtering, and typing e.g. "git"
-- surfaces that whole section. Enter executes the selection.

-- Section order controls how groups appear with an empty query
local sheet_sections = {
  "help", "file", "find", "move", "git", "lsp", "diag",
  "code", "edit", "term", "debug", "ui", "misc",
}

local function sheet_section_of(lhs, text)
  local t = text:lower()
  ...
  if lhs == ",e" or lhs == ",ne" or lhs == ",n<Space>" or lhs == ",n" or lhs == "gx" then return "file" end
  ...
  if t:match("harpoon") or t:match("buffer") or t:match("window") or t:match("resize")
      or t:match("bnext") or t:match("bprevious") or t:match("tabnew") then return "move" end
  if t:match("comment") or t:match("surround") or t:match("outline") or t:match("aerial") or t:match("format") then return "code" end
  if t:match("empty line") then return "edit" end
...
_G.Telescope_shortcut_sheet = function()
  ...
  local rank = {}
  for i, s in ipairs(sheet_sections) do rank[s] = i end
    ...
    for _, km in ipairs(source) do
      ...
      if not lhs:find("<Plug>") and not desc:match("^:help .*%-default") then
        table.insert(keymaps, {
          ...
          lhs = lhs,
          desc = desc,
          section = sheet_section_of(lhs, desc),
        })
      end
    end
  end
